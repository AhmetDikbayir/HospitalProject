...
public class HospitalService {
    ...
    static {
        ...
        try {
            ...
            st.execute("CREATE TABLE IF NOT EXISTS doctors(doctor_id SERIAL PRIMARY KEY, doctor_name VARCHAR(20), doctor_surname VARCHAR(20), doctor_title VARCHAR(20))");
            String patientTable = "CREATE TABLE IF NOT EXISTS patients(patient_id SERIAL PRIMARY KEY, patient_name VARCHAR(20), patient_surname varchar(20), patient_case VARCHAR(20))";
            st.execute(patientTable);

            String sql3 = "CREATE TABLE IF NOT EXISTS doctors_patient(doctor_id INT, patient_id INT, FOREIGN KEY(doctor_id) REFERENCES doctors(doctor_id), FOREIGN KEY(patient_id) REFERENCES patients(patient_id))";
            st.execute(sql3);
